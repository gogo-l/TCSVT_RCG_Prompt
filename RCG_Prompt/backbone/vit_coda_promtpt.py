...
class Block(nn.Module):
    ...
    def forward(self, x, register_hook=False, prompt=None, return_attention=False):
        if return_attention:
            x1, atten = self.attn(self.norm1(x), register_hook=register_hook, prompt=prompt, return_attention=True)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1, prompt=None, q=None, train=False, task_id=None):
        ...
        x = x + self.pos_embed[:, :x.size(1), :]  # 可尝试将最后一层加上去
        x = self.pos_drop(x)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            x, atten = blk(x, register_blk == i, prompt=p_list, return_attention=True)
            #x = blk(x, register_blk == i, prompt=p_list, return_attention=False)
            if i == 1:
                x1 = x.clone()
        ...
        return x, prompt_loss, feature_dict, re_atten
