...
num_workers = 0
networkmodel_network ='network_save/networkvitcoda.pth'
class Learner(BaseLearner):
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                # logits
                logits, prompt_loss = self._network(inputs, total_cls=self._total_classes,cur_task=self._cur_task+1,train=True)
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                outputs = self._network(inputs, total_cls=self._total_classes,cur_task=self._cur_task+1)[:, :self._total_classes]
            predicts = torch.topk(
    ...
    def _compute_accuracy(self, model, loader):
        ...
        for i, (_, inputs, targets) in enumerate(loader):
            inputs = inputs.to(self._device)
            with torch.no_grad():
                outputs = model(inputs,total_cls=self._total_classes,cur_task=self._cur_task+1)[:, :self._total_classes]
