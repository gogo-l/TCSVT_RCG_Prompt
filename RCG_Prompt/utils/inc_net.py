...
class CodaPromptVitNet(nn.Module):
    ...
    def forward(self, x, total_cls,cur_task,pen=False, train=False):
        if self.prompt is not None:
            with torch.no_grad():
                q, _, feature_dict,attenmatrix = self.backbone(x)
                q = q[:,0,:]
            output_esp, n, constraint1=self.prompt.concate(feature_dict, total_cls, cur_task,attenmatrix)
            out, prompt_loss,_1,attenmatrix = self.backbone(x, prompt=self.prompt, q=q, train=train)
            out = out[:,0,:]
        else:
            out,prompt_loss,_, attenmatrix = self.backbone(x)
            out = out[:,0,:]
        out = out.view(out.size(0), -1)
        if not pen:
            out = self.fc(out)
        if self.prompt is not None and train:
            return out, prompt_loss + constraint1
        else:
            return out
